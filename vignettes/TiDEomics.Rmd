...
For **RNA-seq** data with log-transformed counts (e.g., `log2(count + 1)` as used in this tutorial), set `trend = TRUE` to let limma model the mean-variance relationship - low-abundance features typically have higher variance. For **proteomics** (log~2~ intensity) and many other data, the mean-variance trend is typically weak or absent, so `trend = FALSE` (default) is commonly appropriate. However, this should be verified (e.g. with `limma::plotSA()`).
