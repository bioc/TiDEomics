...
#' @param facet_by A character string specifying the variable to
...
plot_distribution <- function(se_obj, facet_by = NULL, fontsize = 8) {
    ...
    if (is.null(facet_by) || facet_by == "Sample") {
        ...
        if (!is.null(facet_by)) {
            ...
            p <- p + theme_custom(base_size = fontsize)
        }
    } else {
        ...
        p <- ggplot(data_long,
            aes(x = value, y = .data[[y_var]], fill = Replicate)) +
            ggridges::geom_density_ridges(color = "black", alpha = 0.3) +
            facet_wrap(vars(.data[[facet_by]]), nrow = 1) +
            ggtitle("Abundance distribution") +
            xlab("Log2 Abundance") +
